...
import { CONCURRENCY_LIMIT, OWNER, REPO, STALE_HOURS_THRESHOLD, SLEEP_BETWEEN_CHUNKS } from "./settings.js";
import { resetApiCallCount, getOldOpenIssueNumbers, getApiCallCount } from "./utils.js";
...
async function main() {
    ...
    console.debug(
        `Fetching issues older than ${filterDays.toFixed(2)} days...`
    );
    ...
    const totalApiCallsForRun =
        searchApiCalls + totalIssueApiCalls;

    const avgTimePerIssue =
        totalCount > 0
            ? totalProcessingTime / totalCount
            : 0;

    console.info("--- Stale Agent Run Finished ---");
    console.info(`Successfully processed ${processedCount} issues.`);
    console.info(
        `Total API calls made this run: ${totalApiCallsForRun}`
    );
    console.info(
        `Average processing time per issue: ${avgTimePerIssue.toFixed(
            2
        )} seconds.`
    );
}
...
async function run() {
    ...
    console.info(
        `Full audit finished in ${(duration / 60).toFixed(
            2
        )} minutes.`
    );
}
