 ...
 * contributing/samples/code_execution/code_execution.
 *
 * Data science agent.
 *
 * Ported as literally as the two APIs allow: same agent name, the same
 * instruction verbatim (including the doubled `{{x=}}` braces, which both
 * runtimes leave untouched because `x=` is not a valid state key), and the
 * same `BuiltInCodeExecutor`, which exists under the same name in adk-js and
 * likewise just appends `{codeExecution: {}}` to the request tools for
 * Gemini 2+.
 */
...
function baseSystemInstruction(): string {
  return `
  # Guidelines
