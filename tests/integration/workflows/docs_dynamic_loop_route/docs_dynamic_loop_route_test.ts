...
/**
 * Runs the docs sample `samples/workflows/dynamic/loop_route` — https://adk.dev/graphs/dynamic/#loop-route
 *
 * A `while` loop that keeps calling an agent until a checker is satisfied, so
 * the model's answer decides how many times the graph runs. It is also the
 * sample most able to pass while demonstrating nothing — given a request the
 * linter accepts first time, the loop never runs — so the assertions require a
 * dirty first pass, a fixer round, and an exit on a clean check rather than on
 * the round cap.
 */
