...
import {A2ARemoteAgent} from '../../src/a2a/a2a_remote_agent.js';
...
describe('A2ARemoteAgent', () => {
  ...
  it('should throw if neither agentCard nor agentCardSource is provided', () => {
    expect(() => new A2ARemoteAgent({name: 'test'})).toThrow(
      'Either agentCard or agentCardSource must be provided',
    );
  });
  ...
  it('should resolve card from URL and send message streaming', async () => {
    ...
    const agent = new A2ARemoteAgent({
      name: 'test-agent',
      agentCardSource: 'https://example.com/card.json',
