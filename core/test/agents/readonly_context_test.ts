...
function makeInvocationContext(
  ...
  return new InvocationContext({
    ...
    a2aMetadata: overrides.a2aMetadata,
...
describe('ReadonlyContext', () => {
  ...
  it('a2aMetadata delegates to the underlying InvocationContext', () => {
    const a2aMetadata = {requestId: 'req-7'};
    const ic = makeInvocationContext({a2aMetadata});
    const ctx = new ReadonlyContext(ic);
    expect(ctx.a2aMetadata).toEqual(a2aMetadata);
  });

  it('a2aMetadata is undefined when not set on InvocationContext', () => {
    const ic = makeInvocationContext();
    const ctx = new ReadonlyContext(ic);
    expect(ctx.a2aMetadata).toBeUndefined();
  });
