...
export class ServiceAccountCredentialExchanger implements BaseCredentialExchanger {
  ...
  async exchange({
    ...
    if (saConfig.useIdToken) {
      return exchangeForIdToken(saConfig);
    }

    return exchangeForAccessToken(saConfig);
  }
}

/**
 * Exchanges a service account auth credential for an ID token.
 */
...
/**
 * Exchanges a service account auth credential for an access token.
 */
