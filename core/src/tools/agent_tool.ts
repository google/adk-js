...
export class AgentTool extends BaseTool {
  ...
  override async runAsync({
    ...
    if (isLlmAgent(this.agent) && isZodObject(this.agent.originalInputSchema)) {
      const result = this.agent.originalInputSchema.safeParse(args);
      if (!result.success) {
    ...
    if (isLlmAgent(this.agent) && this.agent.outputSchema) {
      ...
      if (isZodObject(this.agent.originalOutputSchema)) {
        const result = this.agent.originalOutputSchema.safeParse(parsedOutput);
        if (!result.success) {
