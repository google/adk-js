 ...
 * runs a tool on the strength of that. A `ToolResumePayload` says only that the
 * user answered a question the tool asked; it carries no authority and cannot
...
export interface ToolResumePayload {
  /**
   * The resume values, keyed by the id of the interrupt that asked for each
   * one. JSON-serializable.
   */
