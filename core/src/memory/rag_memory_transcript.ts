...
/**
 * Fuses the event lists that share a timestamp, so one turn retrieved in two
 * overlapping chunks appears once.
 *
 * Lists with no timestamp in common stay separate.
 */
export function mergeOverlappingEventLists(
  eventLists: TranscriptEvent[][],
): TranscriptEvent[][] {
  const merged: TranscriptEvent[][] = [];
  let pending = eventLists;
  while (pending.length > 0) {
    ...
    while (mergeFound) {
      mergeFound = false;
      const remaining: TranscriptEvent[][] = [];
