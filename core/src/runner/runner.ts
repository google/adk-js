...
export class Runner {
   ...
   * @param params.stateDelta An optional state delta to apply to the session.
   * @param params.runConfig The run config for the agent.
   * @param params.beforeRunCallback An optional callback invoked after the
   *     InvocationContext is created but before the agent loop starts. Use this
   *     to inject `temp:` prefixed state or inspect the context before the
   *     first LLM call is made.
   * @yields The events generated by the agent.
   */
    ...
    try {
      yield* runAsyncGeneratorWithOtelContext<Runner, Event>(
        ...
        async function* () {
          ...
          if (params.beforeRunCallback) {
            await params.beforeRunCallback(invocationContext);
          }
