...
export class Runner {
   ...
   *
   * Establishes an interactive session driven by `liveRequestQueue` and yields
   * the resulting events. Live model audio events with `inlineData` are
   * yielded but not appended to the session to avoid persisting raw audio
